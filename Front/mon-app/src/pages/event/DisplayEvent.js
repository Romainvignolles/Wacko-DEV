    ...
    const fetchOneEvent = () => {              //recupération des events
        ...
        axios({
            ...
            .then((res) => {
                ...
                let backgroundTopColor = res.data.topColor
                let r = parseInt(backgroundTopColor.substring(1, 3), 16);
                let g = parseInt(backgroundTopColor.substring(3, 5), 16);
                let b = parseInt(backgroundTopColor.substring(5, 7), 16);
                backgroundTopColor = "RGB(" + r + ", " + g + ", " + b + ", " + 0.8 + ")";

                let backgroundTextColor = res.data.textColor
                let a = parseInt(backgroundTextColor.substring(1, 3), 16);
                let z = parseInt(backgroundTextColor.substring(3, 5), 16);
                let e = parseInt(backgroundTextColor.substring(5, 7), 16);
                backgroundTextColor = "RGB(" + a + ", " + z + ", " + e + ", " + 0.8 + ")";

                document.querySelector(".displayEvent__title").style.backgroundColor = backgroundTopColor;

                let title = document.querySelectorAll(".displayEvent__titleColor")
                for (const elem of title) {
                    elem.style.color = res.data.titleColor
                }
                document.querySelector("#eventDescription").style.backgroundColor = backgroundTextColor
                document.querySelector("#eventDescription").style.color = res.data.paraColor;
                document.getElementById("eventInfo").style.backgroundColor = backgroundTextColor;
                document.getElementById("eventInfo").style.color = res.data.paraColor;

                setSvgColor(res.data.titleColor)
            })
            .catch((err) => {
            })
    }
            ...
            <div className="displayEvent__info">
                ...
                <div id="eventInfo">
                    ...
                    <div className="params" aria-label="Groupeur">
                        <img src="./images/icon/groupeur.png" alt="" />
